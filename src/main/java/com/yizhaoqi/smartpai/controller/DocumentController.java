    ...
    public ResponseEntity<?> previewFileByName(
        ...
        try {
            ...
            if (userId == null) {
                ...
                data.put("fileSize", file.getTotalSize());
                if (isPdf) {
                    String previewUrl = documentService.generatePdfPreviewUrl(file.getFileMd5());
                    if (previewUrl == null) {
                        Map<String, Object> response = new HashMap<>();
                        response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
            ...
            if (isPdf) {
                String previewUrl = documentService.generatePdfPreviewUrl(file.getFileMd5());
                if (previewUrl == null) {
                    LogUtils.logUserOperation(userId, "PREVIEW_FILE_BY_NAME", fileName, "FAILED_GENERATE_PDF_URL");
                    monitor.end("预览失败：无法生成PDF预览链接");
                    Map<String, Object> response = new HashMap<>();
                    response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
                    response.put("message", "无法生成PDF预览链接");
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
